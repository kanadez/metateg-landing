...
$(document).ready(function(){
   ...
   detectscroll();
   setDOMevents();
   //$("#portfolio_panel_div").yoxview({autoHideMenu:false});
});
...
function setDOMevents(){ // задаёт события для DOM-элементов приложения
   ...
   $('#next_a').click(function(e){
      $('html,body').stop().animate({ scrollTop: $('#portfolio').offset().top-120 }, 500, function(){timer_id = setTimeout("showHint('advantages_hint')",1000)});
      e.preventDefault();
      window.history.pushState(null, null, "#portfolio");
   });
