...
function setDOMevents(){ // задаёт события для DOM-элементов приложения
   ...
   $('#order_a, .order_link').click(function(e){
   ...
   $('#next_a').click(function(e){
      $('html,body').stop().animate({ scrollTop: $('#prices').offset().top-120 }, 500, function(){timer_id = setTimeout("showHint('advantages_hint')",1000)});
      e.preventDefault();
      window.history.pushState(null, null, "#prices");
   });
...
function detectscroll(){
   ...
   if (bottom_line > $('#content5_div').offset().top)
      timer_id = setTimeout("showHint('prices_hint')",1000);
}
